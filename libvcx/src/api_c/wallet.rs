...
use crate::{
    ...
    api_vcx::api_global::wallet::{
        export_main_wallet, open_as_main_wallet, wallet_add_wallet_record, wallet_add_wallet_record_tags,
        wallet_close_search_wallet, wallet_configure_issuer, wallet_delete_wallet_record,
        wallet_delete_wallet_record_tags, wallet_fetch_next_records_wallet, wallet_get_wallet_record, wallet_import,
        wallet_open_search_wallet, wallet_update_wallet_record_tags, wallet_update_wallet_record_value,
    },
    errors::{
        error,
...
pub extern "C" fn vcx_create_wallet(
    ...
    execute_async::<BoxFuture<'static, Result<(), ()>>>(Box::pin(async move {
        match api_vcx::api_global::wallet::create_main_wallet(&wallet_config).await {
            Err(err) => {
...
pub extern "C" fn vcx_close_main_wallet(
    ...
    execute_async::<BoxFuture<'static, Result<(), ()>>>(Box::pin(async move {
        match api_vcx::api_global::wallet::close_main_wallet().await {
            Err(err) => {
                error!(
...
pub extern "C" fn vcx_wallet_export(
    ...
    execute_async::<BoxFuture<'static, Result<(), ()>>>(Box::pin(async move {
        ...
        match export_main_wallet(&path, &backup_key).await {
            Ok(()) => {
                let return_code = error::SUCCESS_ERR_CODE;
                trace!(
...
pub extern "C" fn vcx_wallet_set_handle(handle: WalletHandle) -> WalletHandle {
    api_vcx::api_global::wallet::set_main_wallet_handle(handle)
}
